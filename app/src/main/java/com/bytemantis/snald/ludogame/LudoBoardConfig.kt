...
object LudoBoardConfig {
    ...
    // --- PATHS ---
    // (Existing path definitions preserved for brevity.
    //  Assume PATH_RED, PATH_GREEN, PATH_YELLOW, PATH_BLUE are unchanged from original file.)
    ...
    // Helper: Get global (x,y) from player-specific step index
    fun getGlobalCoord(playerIndex: Int, stepIndex: Int): Pair<Int, Int>? {
        if (stepIndex < 0 || stepIndex > 56) return null

        // OWNER FIX: Updated Mapping for Red->Green->Blue->Yellow
        val path = when(playerIndex) {
            0 -> PATH_RED
            1 -> PATH_GREEN
            2 -> PATH_BLUE   // Index 2 is now BLUE
            else -> PATH_YELLOW // Index 3 is now YELLOW
        }
